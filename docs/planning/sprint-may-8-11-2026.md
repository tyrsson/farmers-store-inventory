...
| 1.8 | **Remove `admin.dashboard` rows from `999_seed.sql`** — delete the `acl_resource` INSERT, `acl_privilege` INSERT, `Sales`/`Warehouse` allow rules for `dashboard/read`, and the `admin.dashboard` `acl_route_privilege` row. ⚠ Must be done before first run of new listener code. | 30m | `data/schema/999_seed.sql` |
